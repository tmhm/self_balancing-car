...
{
    u32 clk_hz = (bus_clk_khz * 1000) >> 1;       //bus频率/2      u32 bus_clk_khz;  //外围总线时钟
    u16 mod;
    u8 sc_ps;
    u16 cv;
...
//     FTM_MODE_REG(FTMx[ftmn]) |= FTM_MODE_WPDIS_MASK;     //xw  添加写允许
 ...
 //     FTM_OUTMASK_REG(FTMx[ftmn])=0XFE;  
      ...
      FTM_COMBINE_REG(FTMx[ftmn])=0;      //Function for Linked Channels (FTMx_COMBINE)
      FTM_OUTINIT_REG(FTMx[ftmn])=0;
      FTM_EXTTRIG_REG(FTMx[ftmn])=0;      //FTM External Trigger (FTMx_EXTTRIG)
      FTM_POL_REG(FTMx[ftmn])=0;          //Channels Polarity (FTMx_POL)
                           //0 The channel polarity is active high.
                           //1 The channel polarity is active low.     
      //Set Edge Aligned PWM
      FTM_QDCTRL_REG(FTMx[ftmn]) &=~FTM_QDCTRL_QUADEN_MASK;
      ...
      FTM_INVCTRL_REG(FTMx[ftmn])=0;     //反转控制
      FTM_SWOCTRL_REG(FTMx[ftmn])=0;     //软件输出控制F TM Software Output Control (FTMx_SWOCTRL)
      FTM_PWMLOAD_REG(FTMx[ftmn])=0;     //FTM PWM Load
