...
#define NRF_CE_HIGH()	    PTE6_OUT=1
#define NRF_CE_LOW()	    PTE6_OUT=0			  //CE置低
#define NRF_PCSN_LOW()      PTE4_OUT=0
#define NRF_PCSN_HIGH()     PTE4_OUT=1
...
u8 TX_ADDRESS[5] = {0x35,0x53,0x14,0x14,0x14};  // 定义一个静态发送地址
u8 RX_ADDRESS[5] = {0x35,0x53,0x14,0x14,0x14};
...
void NRF_Init(void)
{
    ...
    gpio_init(PORTE,6, GPO,LOW);           //初始化CE，默认进入待机模式
    //gpio_init(PORTE,27, GPI,LOW);           //初始化IRQ管脚为输入 
    gpio_init(PORTE,4, GPO,HIGH);          //初始化PCSN管脚为输出，低电平选中  
    
#if IS_USE_ISR
    exti_init(PORTE,5, falling_up);        //初始化IRQ管脚为 :下降沿触发，内部上拉
#else
    gpio_init(PORTE,5, GPI,LOW);           //初始化IRQ管脚为输入     
#endif
...
void  NRFdata_Tidy(void)
{
    u8  Temp;
    u8 flagnum=0;
    NRFtx[0]=(s16)(CCD_Err * 10);
    NRFtx[1]=(s16)(g_fCarAngle * 10);     //  (g_nLeftMotorPulseSigma*10);   //
    NRFtx[2]=(s16)CCD_GateValve;     //(s16)(g_fCarSpeed * 10);     //  (g_nRightMotorPulseSigma*10);  //
    NRFtx[3]=(s8)(s16) IntegrationTime;      //  way_flag ;                   //
    NRFtx[4]=(s16)PixelAverageValue;
     
    for(Temp=0;Temp<5;Temp++)
    {
     /* if(Temp==3){
               txbuf[Temp*6+3]=NRFtx[Temp]/10+48;
                if(txbuf[Temp*6+3]==48){txbuf[Temp*6+3]=32;}
            
               txbuf[Temp*6+4]=NRFtx[Temp]%10+48;
      }*/
    //  else
        if(Temp==2 || Temp==3  || Temp==4){     //CCD平均值
              txbuf[Temp*6+2]=NRFtx[Temp]/100+48;
            if(txbuf[Temp*6+2]==48){
              flagnum=1;
              txbuf[Temp*6+2]=32; }
            
               txbuf[Temp*6+3]=NRFtx[Temp]/10%10+48;
            if(txbuf[Temp*6+3]==48  && flagnum){txbuf[Temp*6+3]=32;}
               ...
               txbuf[Temp*6+4]=NRFtx[Temp]%10+48;
                ...
                if(NRFtx[Temp]==0){
                  txbuf[Temp*6+1]=32;
                  txbuf[Temp*6+2]=32;
                  txbuf[Temp*6+3]=48;     //32是空格
                  txbuf[Temp*6+4]=32;
                  txbuf[Temp*6+5]=32;     //48是零
                }
                else if(NRFtx[Temp]>0){
                     txbuf[Temp*6+1]=NRFtx[Temp]/1000+48;
                  if(txbuf[Temp*6+1]==48){txbuf[Temp*6+1]=32;}
                     txbuf[Temp*6+2]=NRFtx[Temp]/100%10+48;
                  if(txbuf[Temp*6+2]==48){txbuf[Temp*6+2]=32; //flagnum=1;
                  }
                  
                     txbuf[Temp*6+3]=NRFtx[Temp]/10%10+48;
                 // if(txbuf[Temp*6+3]==48){txbuf[Temp*5+3]=32;}
                     txbuf[Temp*6+4]=46;    //小数点
                     txbuf[Temp*6+5]=NRFtx[Temp]%10+48;
                }
                else{
                      txbuf[Temp*6+1]=45;       //45负号
                      txbuf[Temp*6+2]=(-NRFtx[Temp])/100%10+48;
                   if(txbuf[Temp*6+2]==48){txbuf[Temp*6+2]=32; //flagnum=1;
                   }
                   
                      txbuf[Temp*6+3]=(-NRFtx[Temp])/10%10+48;
                  // if(txbuf[Temp*6+3]==48){txbuf[Temp*6+3]=32;}   //小数点前面必须有数
                   
                      txbuf[Temp*6+4]=46;     //小数点46
                      txbuf[Temp*6+5]=(-NRFtx[Temp])%10+48;
                }
                flagnum=0;
       }
