...
u16 ad_xwonce(ADCn adcn, ADC_Ch ch, ADC_nbit bit) //采集某路模拟量的AD值
{
    u16 result = 0;
    ASSERT( ((adcn == ADC0) && (ch >= AD8 && ch <= AD18)) || ((adcn == ADC1) && (ch >= AD4a && ch <= AD17)) ) ; //使用断言检测ADCn_CHn是否正常

    adc_xwstart(adcn, ch, bit);	  //启动ADC转换
...
u16 ad_xwmid(ADCn adcn, ADC_Ch ch, ADC_nbit bit)
{
    u16 i, j, k, tmp;
    ASSERT( ((adcn == ADC0) && (ch >= AD8 && ch <= AD18)) || ((adcn == ADC1) && (ch >= AD4a && ch <= AD17)) ) ; //使用断言检测ADCn_CHn是否正常

    //3次ADC转换
    i = ad_xwonce(adcn, ch, bit);
    j = ad_xwonce(adcn, ch, bit);
    k = ad_xwonce(adcn, ch, bit);
...
*************************DSP算平均*************************************************/
extern  s16  VOLTAGE_GYRO;
//extern  u8  AD1_flag;
void ad1_DSP_ave(ADCn adcn, ADC_Ch ch, ADC_nbit bit, u8 N) //dsp均值滤波,N=20.
{
    //q15_t tmpp = 0;
    u8  i;
    s32 sum=0;
 //   q15_t AD_gyro[20];
    ...
    for(i = N; i >  0; i--)
           //AD_gyro[i]= ad_once(adcn, ch, bit);
      sum+=ad_once(adcn, ch, bit);
   // arm_mean_q15(&AD_gyro[0] , 20 , &VOLTAGE_GYRO);
    VOLTAGE_GYRO = (s16)(sum/4);
   // AD1_flag  = 1;
//    VOLTAGE_GYRO = (s16)tmpp;
}

void ad1_XW_ave(ADCn adcn, ADC_Ch ch, ADC_nbit bit, u8 N) //dsp均值滤波,N=20.
{
 ...
 //   for(i = N; i >  0; i--)
  //    sum=ad_xwonce(adcn, ch, bit);

    VOLTAGE_GYRO = (s16)ad_xwonce(adcn, ch, bit);
}

s16 Get_ad1_Offset(ADCn adcn, ADC_Ch ch, ADC_nbit bit, u8 N) //程序开始时，读取陀螺仪的静态偏置
{
    u8  i;
    s32 sum=0;
    ASSERT( ((adcn == ADC0) && (ch >= AD8 && ch <= AD18)) || ((adcn == ADC1) && (ch >= AD4a && ch <= AD17)) ) ; //使用断言检测ADCn_CHn是否正常
...
/*************************************************************************/
//     Master_Adc_Config.STATUS3  = AVGE_ENABLED    //此处为xw所加，用于硬件8次均值采样，
//                                 | ADCO_CONTINUOUS
 //                                | ADC_SC3_AVGS(AVGS_8);
///////////////////////////////////////////////////////////////////////////                                
...
void adc_xwstart(ADCn adcn, ADC_Ch ch, ADC_nbit bit)  //--添加硬件八次平均----
{
