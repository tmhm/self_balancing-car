...
void LCD_Init(void)        
{
	...
	PORTE_PCR5=(0|PORT_PCR_MUX(1));
	PORTE_PCR6=(0|PORT_PCR_MUX(1)); 
	
	GPIOE_PDDR=GPIO_PDDR_PDD(GPIO_PIN(2)|GPIO_PIN(4)|GPIO_PIN(5)|GPIO_PIN(6));
	
  
	GPIOE_PDOR |=  GPIO_PDOR_PDO(GPIO_PIN(2));
	//LCD_CS=1;	//预制SLK和SS为高电平   	
	
	GPIOE_PDOR &= ~GPIO_PDOR_PDO(GPIO_PIN(5));
	LCD_DLY_ms(50);
	GPIOE_PDOR |=  GPIO_PDOR_PDO(GPIO_PIN(5));
...
/*
void  LCD_printf(byte x,byte y,s16 num)
{
  u16 data1; 
if(num>0) LCD_P6x8Str(x,y,"+");
else  LCD_P6x8Str(x,y,"-");
data1=()()
...
/***********************************************************************
* 函数名称：LCD_Write_Num()
* 入口参数：X:行;Y:列;num:变量;N:要显示的变量的位数   y从50开始，最多9位显示
                  显示3位小数
* 出口参数：无
* 修改人  ：xw
* 修改时间：2013/06/2
