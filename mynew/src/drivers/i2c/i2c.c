...
void I2C_init(I2Cn i2cn)
{
    ...
    /* 设置频率 */
    I2C_F_REG(I2Cx[i2cn])  = I2C_F_MULT(3) | I2C_F_ICR(17) ;  // I2C Frequency Divider register (I2Cx_F)  I2C分频寄存器   I2C最大波特率为 400k
    // MULT=00  即  mul = 1
    ...
    I2C_C1_REG(I2Cx[i2cn]) = I2C_C1_IICEN_MASK;    // 使能 IIC1 
 //   I2C_C1_REG(I2Cx[I2Cn]) |= I2C_C1_TX_MASK;       //使能TX
    I2C_C1_REG(I2Cx[i2cn]) |= I2C_C1_MST_MASK;       //使能主机模式
...
void I2C_StartTransmission (I2Cn i2cn, u8 SlaveID, MSmode Mode)
{
    ...
    SlaveID = (u8)(( SlaveID << 1 ) | Mode) ;            //确定写地址和读地址
...
{
    u8 n;
    for(n = 1; n < 50; n++)      //注意，这个数据太小，会导致读取错误。//官方是50次
    {
        asm("nop");
    }
